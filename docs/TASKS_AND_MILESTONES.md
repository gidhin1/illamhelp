...
| Mobile UI E2E (Playwright / Expo Web) | In Progress | Maestro removed because it was slow and unreliable; fast UI-only flows now target the mobile React Native Web rendering. |
...
- Expand fast Playwright-on-Expo-Web mobile coverage for focused auth, jobs, and consent flows.
