...
BRUNO_PREFER_GENERATED_USERS="${BRUNO_PREFER_GENERATED_USERS:-false}"
...
if [[ -z "${SEEKER_TOKEN}" ]]; then
  if [[ -n "${SEEKER_USERNAME}" && -n "${SEEKER_PASSWORD}" ]]; then
    ...
    if [[ "${BRUNO_PREFER_GENERATED_USERS}" == "true" && -n "${CACHED_SEEKER_USERNAME:-}" && -n "${CACHED_SEEKER_PASSWORD:-}" ]]; then
...
if [[ -z "${PROVIDER_TOKEN}" ]]; then
  if [[ -n "${PROVIDER_USERNAME}" && -n "${PROVIDER_PASSWORD}" ]]; then
    ...
    if [[ "${BRUNO_PREFER_GENERATED_USERS}" == "true" && -n "${CACHED_PROVIDER_USERNAME:-}" && -n "${CACHED_PROVIDER_PASSWORD:-}" ]]; then
