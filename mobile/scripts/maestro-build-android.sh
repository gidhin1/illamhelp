...
if [[ "${MAESTRO_CLEAN_BUILD:-false}" == "true" ]]; then
  echo "Cleaning stale Android build output..."
...
GRADLE_TASKS=("assemble${ANDROID_BUILD_TYPE_CAP}")
...
if [[ "${MAESTRO_VERBOSE:-false}" == "true" ]]; then
  ...
  if ! (cd android && ./gradlew "${GRADLE_TASKS[@]}" >"../$BUILD_LOG" 2>&1); then
    echo "Android Maestro build failed. Last 120 lines from $BUILD_LOG:"
    tail -n 120 "$BUILD_LOG" || true
    exit 1
  fi
fi

echo "Android Maestro APK ready for build type: $ANDROID_BUILD_TYPE"
