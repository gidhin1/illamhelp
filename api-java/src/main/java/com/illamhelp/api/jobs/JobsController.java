...
public class JobsController {
  ...
  @GetMapping("/jobs")
  public JobsService.JobPage list(@AuthenticationPrincipal Jwt jwt, @RequestParam(required = false) Integer limit,
      @RequestParam(required = false) String cursor) {
    return jobsService.list(CurrentUser.fromJwt(jwt).userId(), limit, cursor);
  }

  @GetMapping("/jobs/search")
  public List<JobsService.JobRecord> search(@AuthenticationPrincipal Jwt jwt,
      @Valid @org.springframework.web.bind.annotation.ModelAttribute JobsService.SearchJobsRequest request) {
    return jobsService.search(CurrentUser.fromJwt(jwt).userId(), request);
  }

  @PostMapping("/jobs")
  @ResponseStatus(HttpStatus.CREATED)
  public JobsService.JobRecord create(@AuthenticationPrincipal Jwt jwt, @Valid @RequestBody JobsService.CreateJobRequest request) {
    return jobsService.create(CurrentUser.fromJwt(jwt).userId(), request);
  }

  @PostMapping("/jobs/{id}/apply")
  @ResponseStatus(HttpStatus.CREATED)
  public JobsService.ApplicationRecord apply(@AuthenticationPrincipal Jwt jwt, @PathVariable String id,
  ...
  public List<JobsService.ApplicationRecord> listApplications(@AuthenticationPrincipal Jwt jwt, @PathVariable String id) {
    return jobsService.listApplications(id, CurrentUser.fromJwt(jwt).userId());
  }

  @GetMapping("/jobs/applications/mine")
  public List<JobsService.ApplicationRecord> listMyApplications(@AuthenticationPrincipal Jwt jwt) {
    return jobsService.listMyApplications(CurrentUser.fromJwt(jwt).userId());
  }

  @PostMapping("/jobs/applications/{applicationId}/accept")
  @ResponseStatus(HttpStatus.CREATED)
  public JobsService.ApplicationRecord accept(@AuthenticationPrincipal Jwt jwt, @PathVariable String applicationId) {
  ...
  @ResponseStatus(HttpStatus.CREATED)
  public JobsService.ApplicationRecord reject(@AuthenticationPrincipal Jwt jwt, @PathVariable String applicationId,
  ...
  @ResponseStatus(HttpStatus.CREATED)
  public JobsService.ApplicationRecord withdraw(@AuthenticationPrincipal Jwt jwt, @PathVariable String applicationId) {
    return jobsService.withdrawApplication(applicationId, CurrentUser.fromJwt(jwt).userId());
  }

  @PostMapping("/jobs/{id}/booking/start")
  @ResponseStatus(HttpStatus.CREATED)
  public JobsService.JobRecord start(@AuthenticationPrincipal Jwt jwt, @PathVariable String id) {
    return jobsService.startBooking(id, CurrentUser.fromJwt(jwt).userId());
  }

  @PostMapping("/jobs/{id}/booking/complete")
  @ResponseStatus(HttpStatus.CREATED)
  public JobsService.JobRecord complete(@AuthenticationPrincipal Jwt jwt, @PathVariable String id) {
    return jobsService.completeBooking(id, CurrentUser.fromJwt(jwt).userId());
  }

  @PostMapping("/jobs/{id}/booking/payment-done")
  @ResponseStatus(HttpStatus.CREATED)
  public JobsService.JobRecord paymentDone(@AuthenticationPrincipal Jwt jwt, @PathVariable String id) {
    return jobsService.markPaymentDone(id, CurrentUser.fromJwt(jwt).userId());
  }

  @PostMapping("/jobs/{id}/booking/payment-received")
  @ResponseStatus(HttpStatus.CREATED)
  public JobsService.JobRecord paymentReceived(@AuthenticationPrincipal Jwt jwt, @PathVariable String id) {
    return jobsService.markPaymentReceived(id, CurrentUser.fromJwt(jwt).userId());
  }

  @PostMapping("/jobs/{id}/booking/close")
  @ResponseStatus(HttpStatus.CREATED)
  public JobsService.JobRecord close(@AuthenticationPrincipal Jwt jwt, @PathVariable String id) {
    return jobsService.closeBooking(id, CurrentUser.fromJwt(jwt).userId());
  }

  @PostMapping("/jobs/{id}/booking/revoke-assignment")
  @ResponseStatus(HttpStatus.CREATED)
  public JobsService.JobRecord revokeAssignment(@AuthenticationPrincipal Jwt jwt, @PathVariable String id,
      @Valid @RequestBody(required = false) JobsService.ReasonRequest request) {
  ...
  @ResponseStatus(HttpStatus.CREATED)
  public JobsService.JobRecord cancel(@AuthenticationPrincipal Jwt jwt, @PathVariable String id,
      @Valid @RequestBody(required = false) JobsService.ReasonRequest request) {
